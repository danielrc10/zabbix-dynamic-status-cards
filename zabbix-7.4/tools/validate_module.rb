...
check(manifest['version'] == '1.20.0', 'unexpected module version')
...
check(combined_php.include?('MAXIMO_DIAS_VISUAL = 7'), 'visual-history hard safety limit is missing')
check(combined_php.include?('MAXIMO_DIAS_HISTORICO = 31'), 'historical-summary hard safety limit is missing')
check(combined_php.include?('itens_resumos'), 'exact text-summary aggregation is missing')
check(combined_php.include?('MAXIMO_DIAS_ESTATISTICAS = 365'), 'trend safety limit is missing')
check(combined_php.include?('MAXIMO_DIAS_AGREGACAO_DIARIA = 31'), 'daily-aggregation safety limit is missing')
...
check(combined_php.include?('periodo_maximo_dias'), 'history query safety limit is missing')
...
check(combined_php.include?("Fonte dos dados históricos"), 'historical source selector is missing')
...
check(combined_php.include?("AGREGACAO_HISTORICA_QUANTIDADE"), 'historical sample count is missing')
check(combined_php.include?("FONTE_HISTORICA_AUTO"), 'automatic history/trends selection is missing')
check(combined_php.include?("CItemHelper::addDataSource"), 'native Zabbix data-source selection is missing')
check(combined_php.include?("AGREGACAO_HISTORICA_MINIMO"), 'historical period minimum is missing')
check(combined_php.include?("AGREGACAO_HISTORICA_MAXIMO"), 'historical period maximum is missing')
check(combined_php.include?("AGREGACAO_HISTORICA_SOMA_MAXIMOS_DIARIOS"),
...
check(combined_php.include?("consultarExtremosDiarios"), 'calendar-day historical aggregation is missing')
check(combined_php.include?("CWebUser::$data['timezone']"), 'frontend user timezone is missing')
check(combined_php.include?("DateTimeImmutable"), 'timezone-aware calendar-day boundary calculation is missing')
check(combined_php.include?("'periodo_selecionado'"),
  'single-day dashboard ranges must remain indivisible')
check(combined_php.include?('consultarExtremosNoIntervalo'),
  'exact-range daily extrema query is missing')
check(combined_php.include?("AGGREGATE_MAX"), 'daily historical maximum query is missing')
check(combined_php.include?("AGGREGATE_LAST => 'ultimos_diarios'"), 'daily last-value query is missing')
check(combined_php.include?("AGGREGATE_FIRST => 'primeiros_diarios'"), 'daily first-value query is missing')
